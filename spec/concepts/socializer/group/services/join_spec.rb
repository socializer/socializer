      ...
      RSpec.describe Join, type: :service do
        ...
        describe "when the group and person arguments are nil" do
          context ".new should raise an ArgumentError" do
            let(:join) { Join.new(group: nil, person: nil) }
            it { expect { join }.to raise_error(ArgumentError) }
          end
        end

        describe "when the group argument is nil" do
          context ".new should raise an ArgumentError" do
            let(:join) { Join.new(group: nil, person: person) }
            it { expect { join }.to raise_error(ArgumentError) }
          end
        end
        ...
        describe "when the person argument is nil" do
          context ".new should raise an ArgumentError" do
            let(:join) { Join.new(group: group, person: nil) }
            it { expect { join }.to raise_error(ArgumentError) }
          end
        end
